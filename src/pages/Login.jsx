...
function Login() {
	...
	const [password, setpassword] = useState("one");

	const { customer, status, fetch, token, error } = useSelector(
		state => state.auth,
	);
			...
			<div>
				...
				<div className=' w-full h-full flex items-center flex-col bg-teal-700 sm:p-3 px-10 pt-8 pb-28 '>
					<div className='flex justify-between w-full bg-transparent items-center'>
						...
						<Link
							to='/register'
							className=' text-white underline cursor-pointer text-[15px] tracking-[0.020em] bg-transparent'
						>
							Register
						</Link>
					</div>
					<form
						...
						className='max-w-[500px] mt-10 w-full flex flex-col gap-y-5 bg-white rounded-2xl shadow-lg sm:p-6 py-12 px-6 sm:mb-16 '
					>
						<h1 className='text-[30px] font-bold text-stone-900'>
							<span className='text-sky-600 text-[40px]'>D</span>-Shop ~ Login
						</h1>
						<div className='flex gap-1 flex-col-reverse'>
							...
							<input
								...
								className='email border-b-[1.5px] p-2 py-1 focus:outline-none  placeholder:text-stone-500 focus:border-stone-500 border-stone-300 text-[13px] tracking-wide'
								required
							/>
							<label
								htmlFor='email'
								className='email-label text-[13px] text-stone-800 tracking-wide'
							>
								* email
							</label>
						</div>
						...
						<div className='flex gap-x-3 items-center cursor-pointer'>
							<input
								...
								type='checkbox'
								checked={true}
								className=' py-1 accent-sky-500'
								required
							/>
							<label
								htmlFor='check'
								className='text-[13px] tracking-wide cursor-pointer py-1 text-stone-700'
							>
						...
						<div>
							<Link
								to={"/forget-password"}
								className='forget text-[12px] tracking-wide text-right text-stone-600  hover:text-sky-800 underline'
							>
								<p>forget password?</p>
							</Link>
						</div>
						...
						<div className='flex flex-col'>
							<button
								type='submit'
								className='bg-sky-500 rounded-lg mt-3 shadow-lg py-3 w-full text-white text-[13px] tracking-wide'
							>
								Login
							</button>
							<p className=' w-full text-center text-[14px] mt-2'>or</p>
							<p className='bg-teal-700  text-center rounded-md mt-3 shadow-lg py-3 w-full text-white text-[13px] tracking-wide flex items-center gap-x-3 justify-center'>
								Sign in with google <AiOutlineGoogle className='text-2xl' />{" "}
							</p>
							<p className='bg-teal-700 text-center rounded-md mt-3 shadow-lg py-3 w-full text-white text-[13px] tracking-wide flex items-center gap-x-3 justify-center'>
								Sign in with facebook <FaFacebook className='text-2xl' />{" "}
							</p>
						</div>
					</form>
				</div>
